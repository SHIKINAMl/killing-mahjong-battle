    ...
    public static class GameRules
    {
        ...
        public static int GetBaseHan(string yakuName)
        {
            ...
            switch (yakuName)
            {
                ...
                case "混全帯么九":
                case "七対子":
                // **`一気通貫` が抜けていた**（2026-08-27 の調査）。サーバーの `yaku.py` は27役、
                // こちらは26役しか持っておらず、一気通貫だけ -1（不明）を返していた。
                case "一気通貫":
                    return 2;
        ...
        // --- 獲得金・失う金 ---
        //
        // **本編の決着でこれらを使ってはいけない（2026-08-07 に本人が決定）。**
        // 実際の獲得・損失・決着後の血は、すべてサーバーが `liquidation` で送ってくる。
        // クライアントが計算し直すと、強襲のように「獲得を 0 に潰して相手への
        // 追加ダメージへ回す」ような仕様が入ったときに式が合わなくなり、
        // 画面だけが静かに嘘をつく（実際に「5000 × 1 = 0」が出た）。
        //
        // 残してあるのは、**サーバーに訊けない2箇所だけ**:
        //   - `BettingUI` の予想報酬 … 決着前なので、まだ答えが存在しない
        //   - `TutorialManager`      … チュートリアルはサーバーに接続しない
        //
